...
# 维度 6b：逐年有信号日数(行=年,列=配置;值为当年有 ≥1 信号的日历日数)
dim6b_rows = []
for y in years:
    ymask = frame.year.to_numpy() == y
    row = [y]
    for c in ['并集'] + SEEDS:
        row.append(int(frame.loc[masks[c] & ymask, 'event_date'].nunique()))
    n_caldays = int((cal_years == y).sum())
    row.append(n_caldays if n_caldays else '—')
    for k in range(6):  # 并集/S1~S5 各占当年交易日 %
        row.append(f'{row[k + 1] / n_caldays:.1%}' if n_caldays else '—')
    dim6b_rows.append(row)
log('维度6b 完成：逐年有信号日数')
...
md()
md('覆盖率口径："X%日>" 列 = 有信号日中日信号数超过该值的天数恰占 X%（从左到右覆盖率递减、门槛递增）；max = 单日最多。')
md()
md('| 配置 | 事件总数 | 有信号日数 | 其中落在日历内 | 零信号日占比 | 75%日信号数> | 50%日> | 25%日> | 10%日> | 5%日> | 1%日> | max |')
md('|---|---|---|---|---|---|---|---|---|---|---|---|')
for r in dim1_rows:
...
md()
md('覆盖率口径："X%段段长>" = 段长超过该值的段恰占 X%；"X%段间距>" 同理（样本 = 段数−1 个闲置期）；max = 最大值。')
md()
md('| 配置 | 段数 | 75%段段长> | 50%段> | 25%段> | 5%段> | max | 50%段段内信号> | 段内信号max | 75%段间距> | 50%> | 25%> | 5%> | max |')
md('|---|---|---|---|---|---|---|---|---|---|---|---|---|')
for r in dim2_rows:
    md('| ' + ' | '.join(str(x) for x in r) + ' |')
md()
...
md()
md('覆盖率口径："X%日在持>" = 在持笔数超过该值的日历日恰占 X%（分母含在持为 0 的日子）；max = 峰值。')
md()
md('| 配置 | closed笔数 | 50%日在持> | 10%日> | 5%日> | 1%日> | max | 峰值日期 | 在持为0的日历日占比 |')
md('|---|---|---|---|---|---|---|---|---|')
for r in dim3_rows:
    md('| ' + ' | '.join(str(x) for x in r) + ' |')
md()
...
md()
md('覆盖率口径："X%间隔>" = 同股相邻触发间隔（交易日）超过该值的样本恰占 X%。')
md()
md('| 配置 | 涉及股票数 | 间隔样本数 | 75%间隔> | 50%间隔> | 25%间隔> | 间隔≤20交易日占比 | 同股真实重叠笔数 | 占closed笔数比 |')
md('|---|---|---|---|---|---|---|---|---|')
for r in dim4_rows:
    md('| ' + ' | '.join(str(x) for x in r) + ' |')
md()
...
md()
md('覆盖率口径："X%段>" = 段首日前指数收益超过该值的段恰占 X%。')
md()
md('| 指标 | 75%段> | 50%段> | 25%段> |')
md('|---|---|---|---|')
...
for r in dim6_rows:
    md('| ' + ' | '.join(str(x) for x in r) + ' |')
md()
md('逐年**有信号日数**（当年有 ≥1 信号的日历日数，按 event_date 自然口径；"X%" 列 = 该配置有信号日数 / 当年市场日历交易日数，1992/1993/2026 为不完整日历年）：')
md()
md('| 年份 | 并集 | S1 | S2 | S3 | S4 | S5 | 交易日数 | 并集% | S1% | S2% | S3% | S4% | S5% |')
md('|---|---|---|---|---|---|---|---|---|---|---|---|---|---|')
for r in dim6b_rows:
    md('| ' + ' | '.join(str(x) for x in r) + ' |')
md()
