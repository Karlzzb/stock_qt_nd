...
3. 2026-09-09 全量完工实测(修订记录,正文不动):
   ① 全量主跑(基模锚定 → 层1~层4 → 定版打分 → 产物落盘)43.5 秒,--repro-check 全新进程全链重导 41 秒逐位一致(层表/拐点/终选/model.txt 与 base_model.txt 字节/分数数组),合计 86.5 秒,远优于 §七 预估(20~60 分钟,按 M3 实测外推的保守值);
   分数双跑 md5 一致(f426a0efaaffcc8cfd24e941b4018274)。
   ② 五层计数:层1 1,997 → 221(零重要性剔 1,776);层2 221 → 194(漂移剔 27,年份 2020~2023);层3 194 → 144(簇吸收 50);层4 阶梯 [5,10,15,20,25,30,40,50,75,100,144] 全出数,拐点 K*=10。
   ③ 定版:终选 10 特征,终模 28 轮,val 头部五名净笔均(日加权)+0.017732 / cluster_t +3.433(基模全特征 +0.016313 / +3.289);scores_v6.parquet 96,577 行 × 5 列(event_id/ts_code/date/seg/score,无标签列),test 17,902 行仅出分数零指标。
   ④ 独立自检(selfcheck_m4.py,层5 口径自写重算,不调 feature_selection 选择函数与驱动函数)22.9 秒全绿:当选解析/段界/泄漏重扫/基模折外对 M3 逐位/层1~层4 自写重算全等/定版分数自写重算逐位/md5 台账/test 零触碰,all_pass=true。
   ⑤ 自检渲染修正一处(报告排版,非数据/口径):层4 曲线表拐点标记由行末附列改为 K 单元格内联,保持列数与表头一致。
