...
- 2026-09-09 全量完工实测(修订记录,正文不动):
  ① 标签构建双跑合计 33.9 秒(含涨跌停 4,778 文件加载 8s ×2 与模拟 9s ×2),双跑 md5 逐位一致(0180da04cc87ea894c85ab5ecf2ab0a9)。
  ② 全量跑赛 8×36 = 288 配置实测 3,248.4 秒(约 54 分钟,workers=3),含八候选当选配置 OOF 复现断言(stored_vs_rerun)全部逐位一致;总机时与冒烟预估(35~60 分钟)一致。
  ③ 自检实现修正一处(自检代码 bug,非数据/口径问题):selfcheck_m3 当选配置 OOF 独立复跑初版在排序前的 train 行序上取标签向量再布尔掩码,与排序后的特征矩阵行序错位,导致独立复跑与落盘折外分数不一致;改为在排序后的 sub 上重取标签后,八候选 OOF 独立复跑逐位一致全过。
  该 bug 只影响自检脚本自身,跑训驱动产出的折外分数与指标表不受影响(驱动内 stored_vs_rerun 断言首跑即全过)。
  ④ 裁决落盘(待监工独立复核,status=provisional_pending_independent_review):当选 = net_ret_60d(次日开盘入场净收益幅度回归标签,60 个交易日视野),当选配置 config_id=24(num_leaves=63, min_data_in_leaf=50, learning_rate=0.05, feature_fraction=0.6),val 头部五名净笔均(日加权)= +0.016313,val cluster_t = +3.289,双约束全过(八候选中位数 +0.004084);H=60 双头为唯二过 cluster_t≥2 的候选。
