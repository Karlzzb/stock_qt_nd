#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""背离v6 特征工程+模型战役 M5:阶段门审判(模型路线生死关) —— 预登记 = 同目录 README.md(冻结,先于跑数落盘,勿改)。

驱动 = #40 底座 experiments/v6_long_hold_trial/run_long_hold.py 只读 import 复用
(本体零改动,启动时 md5 校验 == a8f1cca6d2bad167899454f896f6d026)+ 本文件适配层
(适配点与预登记 §三 逐条对应,全部集中在适配段,逐一注释标记 "M5 适配")。

网格(README §一.4,18 格):挑选规则 {M 模型分数排序} ∪ {S1~S5} × K ∈ {3,5,10},
池 = ALL(seg=='val' 24,405 事件),H = 60,仓位 = P7K{K},出场 = E1_A6(tp=0.25, sl=-0.18,断言守护)。
事件窗口 2020-01-01~2023-12-31(seg=='val' 子集);回测日历延伸至 2023-12-31 后第 90 个
上证指数交易日(2024-05-21,H=60 视野 + 顺延缓冲,README §二 披露);硬断言全 18 格
truncated_window == 0。

主门(生死,#41 第二轮裁决第 4 条原文):任一 K 档 M 格净笔均 ≥ max(S1~S5 同 K 档) + 2pp
且该格 cluster_t ≥ 2 → 过门进 M6;三档全不过 → 模型路线判死。
副门(信息性):precision@top10%(val 有效标签 24,247 起全段 pooled,前 ⌈10%⌉ = 2,425 名)
相对池基线提升。

自检(README §四):#40 机制一致性对账(S1~S5 × K3 五格,交集逐笔市场机制字段浮点差 0)、
成交单 vs 标签同机制子集对账(容差 1e-9)、模型分数接入断言(键一对一/val 全覆盖/
标签 NaN 事件不得成交(dropped_cash 本金口径例外计数披露)/embargo/test 零开仓)、
S4 格与全 18 格双跑逐位一致(detcmp_m5.log)、底座 md5 校验、引擎内置断言原样继承。

用法:
    python3 run_stage_gate_m5.py --mode full          # 加载 + 对账 + 18 格双跑 + 宣判 + 报告
    python3 run_stage_gate_m5.py --mode emit --only <cid,cid> --emit-dir <dir>  # 自检用:单格重跑落盘
"""
...
M5_DIR = REPO / "experiments" / "v6_model_campaign" / "m5_stage_gate"
...
LH_RUNS = REPO / "experiments" / "v6_long_hold_trial" / "runs"  # #40 冻结产物(只读)
CKPT_PATH = M5_DIR / "checkpoint_pass1.pkl"  # 断点续跑(*.pkl 按 .gitignore 不入库)

BASE_MD5 = "a8f1cca6d2bad167899454f896f6d026"  # run_long_hold.py 冻结校验值(README §四.5)
...
def log(msg: str) -> None:
    RLH.log(f"[m5_stage_gate] {msg}")


# ---------------------------------------------------------------- M5 适配 §三.3:模型挑选规则
def m5_select_chosen(pg: dict, sig_idx: np.ndarray, n_free: int, rule: str,
...
# ---------------------------------------------------------------- 网格(README §一.4)
def enumerate_grid_m5() -> list[dict]:
    ...
    for sel in SELS6:
        for K in K_LIST_M5:
            cells.append(RLH.make_cfg("ALL", H_M5, sel, f"P7K{K}", EXIT_M5))
...
# ---------------------------------------------------------------- 自检 §四.4:双跑逐位对拍
def detcmp_m5(cells: list[dict], ckpt: dict, pass2: dict) -> dict:
    ...
    with open(RLH.DETCMP_PATH, "w", encoding="utf-8") as f:
        f.write("M5 阶段门审判:全 18 格双跑逐位确定性对拍(README §四.4)\n")
    ...
    log(f"[detcmp] 18 格双跑逐位对拍(含 S4 三档),不一致 {len(bad)} 格"
        f" -> {'PASS' if ok else 'FAIL'}(落 detcmp_m5.log)")
...
def reconcile_vs_40(val_keys: set) -> dict:
    ...
    cfg_deg["cell_id"] = "DEGENERATE__M5__P1_E1A6_S1_H60"
...
def main_gate(metrics: dict) -> dict:
    ...
    log(f"[主门宣判] {'过线 —— 进 M6' if overall else '三档全不过 —— 模型路线判死'}")
    return dict(per_k=per_k, passed=bool(overall),
                verdict=("PASS_TO_M6" if overall else "MODEL_ROUTE_DEAD"),
                clause=("#41 第二轮裁决第 4 条:模型排序爆发日 top-K 净笔均 ≥ 最好手工挑选规则 "
                        "+2pp 且 cluster_t ≥ 2;全标签全配置不过 → 模型路线判死,回用户拍板"))
...
def write_summary(metrics: dict) -> pd.DataFrame:
    ...
    df.to_csv(M5_DIR / "summary_stage_gate_m5.csv", index=False)
    log(f"[dump] summary_stage_gate_m5.csv {len(df)} 行(全精度)")
    return df
...
def write_report(metrics: dict, gate: dict, secondary: dict, checks: dict,
                 elapsed: float) -> None:
    L: list[str] = []
    L.append("# M5 阶段门审判(模型路线生死关)· 裁决报告")
    L.append("")
    ...
    L.append("预登记军令状 = 同目录 README.md(冻结,先于任何跑数落盘);机器底座 = #40 "
    ...
    L.append(f"1. 引擎口径回归(#40 机制一致性):退化格(P1×E1_A6×S1×H60,上限无穷+现金放开)"
             ...
             f"(窗口不同所致组合状态差,预期内)逐格披露于 verdict_stage_gate_m5.json。")
    ...
    L.append("")
    # ---- 3. 主门宣判 ----
    L.append("## 3. 主门宣判(生死门,#41 第二轮裁决第 4 条原文执行)")
    L.append("")
    ...
    if gate["passed"]:
        ...
        L.append(f"**主门宣判:过线(K 档 {', '.join(passed_ks)})—— 模型路线过阶段门,进 M6 终审。**")
    else:
        L.append("**主门宣判:三个 K 档全不过 —— 模型路线判死,如实落盘,回用户拍板下一方向"
                 "(#47 M5 节判死条款)。**")
    L.append("")
    ...
    L.append("|---|---|---|---|---|---|---|")
    for K in K_LIST_M5:
        vals = [metrics[f"ALL__H60__{s}__P7K{K}__E1_A6"]["net_avg"] for s in SELS6]
    ...
    L.append("|---|---|---|---|")
    for s in SELS6:
        vals = [metrics[f"ALL__H60__{s}__P7K{K}__E1_A6"]["cluster_t"] for K in K_LIST_M5]
    ...
    L.append(f"- 信号池价值(不涉排序):池基线精度 {secondary['pool_baseline'] * 100:.2f}%"
             ...
             f"不作本阶段输入)。")
    best_s_overall = max(
        (metrics[f"ALL__H60__{s}__P7K{K}__E1_A6"]["net_avg"], s, K)
        for s in ("S1", "S2", "S3", "S4", "S5") for K in K_LIST_M5
        if metrics[f"ALL__H60__{s}__P7K{K}__E1_A6"]["net_avg"] is not None)
    ...
    L.append("7. #40 机制一致性对账的接入集合差(交集外两侧独有笔数)为窗口不同的预期结果,"
             "逐格披露于 verdict_stage_gate_m5.json,不构成口径分歧。")
    L.append("")
    L.append(f"总耗时 {elapsed:.0f} 秒;产物 = summary_stage_gate_m5.csv(18 格全精度)/ "
             f"verdict_stage_gate_m5.json / detcmp_m5.log / runs/(每格 equity+trades+stats)。")
    with open(M5_DIR / "report.md", "w", encoding="utf-8") as f:
        f.write("\n".join(L))
    log("[dump] report.md")
...
# ---------------------------------------------------------------- 主流程
def main() -> None:
    ap = argparse.ArgumentParser(description="M5 阶段门审判(预登记 README.md)")
    ...
    RLH.OUT_DIR = M5_DIR
    RLH.RUNS_DIR = M5_DIR / "runs"
    RLH.LOG_PATH = M5_DIR / "progress.log"
    RLH.DETCMP_PATH = M5_DIR / "detcmp_m5.log"
    ...
    if args.mode == "emit":
        ...
        _HB["stop"] = True
        log(f"M5 EMIT DONE ({time.time() - t_all:.0f}s)")
        return

    # ---- 第一遍(断点续跑,逐格落盘)+ 第二遍(确定性重算)----
    ckpt = run_pass1(cells)
    pass2 = run_pass2(cells)
    checks["check_detcmp"] = detcmp_m5(cells, ckpt, pass2)
    ...
    all_ok = all(v.get("ok", True) for v in checks.values())
    verdict = dict(
        experiment="背离v6 特征工程+模型战役 M5:阶段门审判(模型路线生死关)",
        issue=52, part_of=47,
    ...
    with open(M5_DIR / "verdict_stage_gate_m5.json", "w", encoding="utf-8") as f:
        json.dump(verdict, f, ensure_ascii=False, indent=2, default=str)
    log("[dump] verdict_stage_gate_m5.json")
    write_report(metrics, gate, secondary, checks, time.time() - t_all)

    _HB["stop"] = True
    log(f"M5 STAGE GATE DONE ({time.time() - t_all:.0f}s) "
