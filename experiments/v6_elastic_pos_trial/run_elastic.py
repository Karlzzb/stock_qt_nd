#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""背离v6 R1 弹性仓位(P5) + 口径D 终审 —— 预登记 = 同目录 README.md(冻结,先于跑数落盘,勿改)。

本脚本 = #32 底座 experiments/v6_portfolio_trial/run_portfolio.py 逐字拷贝 + P5 增量
(增量处逐行注释标记 "P5 增量")。唯一执行脚本。输入输出同 #32;
输出 = 每格 equity_curve/trades/stats + summary_elastic.csv + verdict_elastic.json
+ report.md + detcmp.log(全量双跑逐位对拍)。

网格 = 池(6: v6-1~v6-5 + ALL) × H(10/20/25) × 出场(35,逐字 #32 §3.1) × P5 = 630 格;
sel 维不存在(P5 无挑选对象),cell_id 的 sel 段固定写 "S0"。
...
P5 增量解释性决断(本实验新增,规格未钉死处,全部披露,不改规格):
 E-1. P5 的"当日池内信号数 S" = 当日池内全部信号数 len(sig_idx),含无下一交易行
    的 truncated_no_next 信号(其不入 pending、不占现金;摊薄分母偏保守,方向安全)。
 E-2. P5 现金再摊薄所用 cash = 信号日收盘决策时点的现金(= 上一交易行收盘后现金;
    #32 执行语义下信号日收盘至入场日开盘无现金变动,因果干净且与遍历顺序无关)。
 E-3. slot_budget 拿不到 cash,P5 单仓金额在信号日挑选块内联计算并随 pending 传给
    入场执行块;slot_budget 的 P5 分支为防御性 raise(结构性不可达)。
 E-4. P5 遍历顺序 = ts_code 升序,由事件表按 (event_date, ts_code) mergesort 排序 +
    groupby 保序结构性保证(与 #32 S1 先到先得代理同源)。
 E-5. capital_utilization == 0 时 per_cash/per_idx 记 NaN(防御;判活线含 u>=0.5,
    u=0 格必不过线)。
 E-6. 确定性自检(README §五.4) = 全量 630 格双跑,逐格 summary+stats(除 runtime_sec)
    逐位对拍落 detcmp.log;equity/trades 逐位由抽样 selfcheck_determinism 深度验证
    (沿用 #32 框架)。
 E-7. summary_elastic.csv 的 passed 列 = 主线口径 D 纯现金字面版;两个参考列名单只在
    verdict_elastic.json / report.md 出数。
 E-8. 旧终审线参考列 = excess_idx > +15pp 且 sharpe_idx > 0.5;#32 旧线的 S5 附加
    cluster_t 条款对 S0 无操作对象,按字面只取这两条。
 E-9. 全市场 ATR 磁盘缓存只读复用 #32 的确定性产物(cache/mkt_atr_lb{14,21}.parquet;
    缓存缺失时按 #32 同算法确定性重建)。
E-10. 报告 P5 vs P1/P2 对照聚合粒度 = 池×H(630 格明细在 summary_elastic.csv;
    P1/P2 数字取自 #32 冻结产物 summary_portfolio.csv,不重跑)。

用法:
    python3 run_elastic.py --mode calibrate   # 自检 1:#32 冻结 summary 口径 D 两版本复算
    python3 run_elastic.py --mode selfcheck   # 自检 1 + 继承对账自检 + 抽样确定性
    python3 run_elastic.py --mode benchmark   # 自检 + 8 抽样格计时(两遍,确定性)
    python3 run_elastic.py --mode full        # 全量 630 格双跑 + 终审出数(监工派活)
"""
...
OUT_DIR = Path(os.path.join(REPO, "experiments", "v6_elastic_pos_trial"))
BASE_DIR = Path(os.path.join(REPO, "experiments", "v6_portfolio_trial"))  # P5 增量:#32 底座目录(冻结产物只读)
CACHE_DIR = BASE_DIR / "cache"   # P5 增量:mkt_atr 缓存只读复用 #32 确定性产物(决断 E-9)
BASE_SUMMARY_PATH = BASE_DIR / "summary_portfolio.csv"  # P5 增量:#32 冻结 summary(自检 1/3 与对照用)
RUNS_DIR = OUT_DIR / "runs"
LOG_PATH = OUT_DIR / "progress.log"
DETCMP_PATH = OUT_DIR / "detcmp.log"  # P5 增量:全量双跑逐位对拍日志(README §五.4)
...
SEL_FIXED = "S0"          # P5 增量:P5 无挑选对象,sel 段固定写 S0(README §三.1)
POS_FIXED = "P5"          # P5 增量:本实验唯一仓位规则(README §三.1)
S4_SEED = 42
...
# 基准抽样 8 格(施工计时用,覆盖全部出场族,确定性硬编码;P5 增量:pos=P5 sel=S0)
BENCHMARK_CELLS = [
    ("ALL", 20, "S0", "P5", "E0"), ("ALL", 20, "S0", "P5", "E1_A5"),
    ("ALL", 20, "S0", "P5", "E2_B1"), ("ALL", 20, "S0", "P5", "E3"),
    ("ALL", 20, "S0", "P5", "E4"), ("v6-5", 20, "S0", "P5", "E5"),
    ("v6-5", 10, "S0", "P5", "E6"), ("v6-1", 25, "S0", "P5", "E7"),
]
...
def slot_budget(cfg: dict, pg: dict, ev_i: int, day: int, cap: int,
    ...
    if pos == "P5":
        # P5 增量:P5 单仓金额 = min(100万/max(10,S), cash/S),cash 在信号日挑选块内
        # P5 增量:才拿得到,故金额在挑选块内联计算(决断 E-3);本分支结构性不可达。
        raise AssertionError("P5 单仓金额在信号日挑选块内联计算,slot_budget 不可达")
...
def run_cell(cfg: dict) -> dict:
    ...
    # P5 增量:爆发日接入与摊薄统计(README §六 report 用;signal_* 为池级常量,格内同值)
    _sizes = [len(v) for v in pg["by_date"].values()]  # P5 增量
    stats["signal_days"] = len(_sizes)                 # P5 增量
    stats["signal_days_burst"] = int(sum(1 for s in _sizes if s > 10))  # P5 增量:S>10
    stats["signal_max"] = int(max(_sizes)) if _sizes else 0             # P5 增量
    stats["p5_days_diluted"] = 0     # P5 增量:名义摊薄(S>10)信号日计数
    stats["p5_days_cashbound"] = 0   # P5 增量:现金再摊薄(cash/S 紧于名义)信号日计数
    ...
    for di in range(len(cal)):
        ...
        if sig_idx is not None:
            ...
            if degenerate:
                cap = 10 ** 9
            if cfg["pos"] == "P5":
                cap = 10 ** 9  # P5 增量:全部接入,无仓位数上限;入场时二次检查同此 cap,恒放开
            n_free = cap - len(positions)
            if n_free < len(sig_idx):
                stats["dropped_slot_full"] += len(sig_idx) - max(n_free, 0)
            if cfg["pos"] == "P5":
                # P5 增量:无挑选对象,当日池内信号全部接入;遍历顺序 = ts_code 升序
                # P5 增量:(by_date 由 (event_date, ts_code) 排序的事件表 groupby 保序构造,决断 E-4)
                chosen = sig_idx
                # P5 增量:单仓金额 = min(100万/max(10,S), 当前 cash/S),当日所有信号同一金额。
                # P5 增量:S = 当日池内全部信号数(含 truncated_no_next,决断 E-1);
                # P5 增量:cash = 信号日收盘后可用现金(上一交易行收盘值,决断 E-2);
                # P5 增量:slot_budget 拿不到 cash,此处内联(决断 E-3)。
                S_n = len(sig_idx)  # P5 增量
                nominal_p5 = INIT / max(10, S_n)  # P5 增量:名义金额 = 100万/max(10,S)
                budget_p5 = min(nominal_p5, cash / S_n)  # P5 增量:现金再摊薄
                if S_n > 10:
                    stats["p5_days_diluted"] += 1  # P5 增量:名义摊薄日
                if cash / S_n < nominal_p5:
                    stats["p5_days_cashbound"] += 1  # P5 增量:现金再摊薄日
            else:
            ...
            for ev_i in chosen:
                ...
                if ed < 0:
                    ...
                    continue
                if cfg["pos"] == "P5":
                    budget = budget_p5  # P5 增量:当日所有信号同一金额,随 pending 传入场执行块
                else:
    ...
    # P5 增量:单位在投超额 = 净年化超额 / 资金利用率,双口径各算一份(README §四);
    # P5 增量:u=0 防御记 NaN(决断 E-5)。
    util = float(equity["market_value"].mean() / INIT)
    per_cash = float(excess_cash / util) if util > 0 else np.nan  # P5 增量
    per_idx = float(excess / util) if util > 0 else np.nan        # P5 增量
    # P5 增量:passed = 主线判活线 = 口径 D 纯现金字面版(README §一.2,决断 E-7)
    ...
    stats.update(dict(
        ...
        per_cash=per_cash, per_idx=per_idx,  # P5 增量:单位在投超额双口径(README §四)
...
def summary_row(cfg: dict, stats: dict) -> dict:
    ...
    for k in ("total_signals", "entered", "n_trades", "dropped_slot_full",
              ...
              "p5_days_diluted", "p5_days_cashbound"):  # P5 增量:末五列为摊薄统计
        row[k] = stats[k]
...
def enumerate_grid() -> list[dict]:
    """P5 增量:网格 = 池(6) × H(3) × 出场(35) × P5 = 630 格,sel 固定 S0(README §三.2)。"""
    cells: list[dict] = []
    for pool in POOLS:
        for H in H_LIST:
            for ex in EXIT_CONFIGS:
                cells.append(make_cfg(pool, H, SEL_FIXED, POS_FIXED, ex["name"]))
    return cells
...
# ---------------------------------------------------------------- 自检 1:口径复算(README §五.1)
def selfcheck_caliber() -> dict:
    """独立从 #32 冻结 summary_portfolio.csv 重算口径 D 两版本过线格数。

    纯现金字面版必须 0 格、混合口径变体必须 39 格,两数逐位复现才继续,否则停工。
    """
    set_stage("check0: 口径复算自检(#32 冻结 summary_portfolio.csv)")
    df = pd.read_csv(BASE_SUMMARY_PATH)
    assert len(df) == 12240, f"#32 冻结 summary 行数 {len(df)} != 12240"
    ...
    n_pure, n_mix = int(m_pure.sum()), int(m_mix.sum())
    ok = (n_pure == 0 and n_mix == 39)
    log(f"[check0] 口径D 纯现金字面版过线 {n_pure} 格(预期 0);"
        f"混合口径变体过线 {n_mix} 格(预期 39) -> {'PASS' if ok else 'FAIL'}")
    return dict(ok=bool(ok), n_pure=n_pure, n_mixed=n_mix,
                expect_pure=0, expect_mixed=39, n_rows=int(len(df)),
                note="独立重算自 #32 冻结 summary_portfolio.csv(README §五.1)")


# ---------------------------------------------------------------- 自检 2/3:P5 无拒仓 + 单调性(README §五.2/§五.3)
def selfcheck_p5(rows: list[dict]) -> dict:
    """全部格 dropped_slot_full==0(硬断言);同(池,H,出场)下若 P5 格 dropped_cash==0
    则 entered_P5 >= #32 冻结 summary 中同配置 P1 五个 sel 格各自的 entered(硬断言);
    dropped_cash>0 的格计数披露。"""
    set_stage("checkP5: 无拒仓 + 对 #32 P1 单调性")
    base = pd.read_csv(BASE_SUMMARY_PATH)
    p1 = base[base["pos"] == "P1"]
    p1_entered = {(r.pool, int(r.H), r.sel, r.exit): int(r.entered)
                  for r in p1.itertuples(index=False)}
    assert len(p1_entered) == 6 * 3 * 5 * 35
    bad_slot: list[str] = []
    bad_mono: list[str] = []
    n_cash_drop = 0
    tot_cash_drop = 0
    for r in rows:
        ...
        assert s["dropped_slot_full"] == 0, \
            f"自检2 失败:{cid} dropped_slot_full={s['dropped_slot_full']} != 0"
        if s["dropped_slot_full"] != 0:
            bad_slot.append(cid)
        if s["dropped_cash"] > 0:
            n_cash_drop += 1
            tot_cash_drop += int(s["dropped_cash"])
            continue  # 现金丢弃格不参与单调性断言(README §五.3)
        for sel in ("S1", "S2", "S3", "S4", "S5"):
            e1 = p1_entered[(summ["pool"], int(summ["H"]), sel, summ["exit"])]
            assert s["entered"] >= e1, \
                f"自检3 失败:{cid} entered={s['entered']} < P1/{sel} entered={e1}"
            if s["entered"] < e1:
                bad_mono.append(f"{cid} entered={s['entered']} < P1/{sel}={e1}")
    ok = not bad_slot and not bad_mono
    log(f"[checkP5] 无拒仓:全部 {len(rows)} 格 dropped_slot_full==0;"
        f"单调性:dropped_cash==0 的 {len(rows) - n_cash_drop} 格对 P1 五 sel 格全满足;"
        f"dropped_cash>0 格 {n_cash_drop} 个(合计丢弃 {tot_cash_drop} 起,披露)"
        f" -> {'PASS' if ok else 'FAIL'}")
    return dict(ok=bool(ok), n_cells=len(rows),
                n_dropped_slot_full_violation=len(bad_slot),
                n_monotonicity_violation=len(bad_mono),
                n_dropped_cash_cells=n_cash_drop,
                total_dropped_cash=tot_cash_drop,
                bad_slot_head=bad_slot[:10], bad_mono_head=bad_mono[:10])


# ---------------------------------------------------------------- 自检 4(README §五.4):全量双跑逐位对拍
def detcmp_full(rows1: list[dict], rows2: list[dict]) -> dict:
    """两遍全量逐格对拍:summary 与 stats(除 runtime_sec)严格逐位一致,落 detcmp.log。"""
    ...
    with open(DETCMP_PATH, "w", encoding="utf-8") as f:
        f.write("R1 弹性仓位(P5) 全量双跑逐位确定性对拍(README §五.4)\n")
...
DISCLOSURE_S7 = [  # README §七 预写披露,出数后原样保留(逐字)
    ...
    "P5 在爆发日可把全部可用现金投入(单仓摊薄至现金 ÷ S),组合波动与回撤结构会与 "
    "P1~P4 不同;口径 D 的 sharpe_cash 条件即对此的风险约束,不另设杠杆上限。",
    "口径 D 为拍板的新判活线,旧终审线(+15pp 且 Sharpe>0.5,判活口径记账)只作参考列"
    "出数;本实验不回溯改判 #32 已归档结论。",
    "#35 的\"39 格过线\"系口径误算(§二),本实验以其纯现金字面版为终审线,39 格数仅作"
    "混合口径参考列复现留存。",
]
...
def write_summary_verdict_report(rows: list[dict], checks: dict, bench: dict,
    ...
    summary.to_csv(OUT_DIR / "summary_elastic.csv", index=False, float_format="%.6f")
    log(f"[dump] summary_elastic.csv {len(summary)} 行({scope})")
    ...
    verdict = dict(
        experiment="背离v6 R1 弹性仓位(P5) + 口径D 终审",
        ...
        ref_lines=["混合口径变体(参考,不判活): excess_idx/u > +10pp 且 u>=0.5 "
                   "且 sharpe_cash > 0.5(README §二)",
                   "旧终审线(参考,不判活): excess_idx > +15pp 且 sharpe_idx > 0.5"
                   "(S5 附加 cluster_t 条款对 S0 无操作对象,决断 E-8)"],
    ...
    with open(OUT_DIR / "verdict_elastic.json", "w", encoding="utf-8") as f:
        json.dump(verdict, f, ensure_ascii=False, indent=2, default=str)
    log("[dump] verdict_elastic.json")
    write_report(summary, verdict, checks, bench, scope)


# ---------------------------------------------------------------- P5 vs P1/P2 对照(数字取自 #32 冻结 summary,不重跑)
...
def write_report(summary: pd.DataFrame, verdict: dict, checks: dict, bench: dict,
    ...
    base = load_base_p1p2()
    L: list[str] = []
    L.append("# 背离v6 R1 弹性仓位(P5) + 口径D 终审 · 裁决报告")
    L.append("")
    L.append(f"范围:{scope}。预登记军令状 = 同目录 README.md(冻结,先于任何跑数落盘)。")
    L.append("本报告全部数字由脚本从产物重算生成;P1/P2 对照数字取自 #32 冻结产物 "
             "summary_portfolio.csv,不重跑。")
    L.append("")
    ...
    L.append(f"1. 口径复算自检:从 #32 冻结 summary 独立重算——纯现金字面版过线 "
             f"{c0.get('n_pure')} 格(预期 0)、混合口径变体过线 {c0.get('n_mixed')} 格"
             f"(预期 39) -> {'PASS' if c0.get('ok') else 'FAIL'}")
    ...
    cp5 = checks.get("checkP5", {})
    n_mono = (cp5.get("n_cells", 0) or 0) - (cp5.get("n_dropped_cash_cells", 0) or 0)
    L.append(f"3. P5 无拒仓断言:全部 {cp5.get('n_cells')} 格 dropped_slot_full == 0(硬断言);"
             f"单调性断言:dropped_cash==0 的 {n_mono} 格对 #32 同配置 P1 五 sel 格各自的 "
             f"entered 全满足 entered_P5 ≥ entered_P1(硬断言;满足前提的格为 0 时断言真空成立,"
             f"如实披露);dropped_cash>0 格 {cp5.get('n_dropped_cash_cells')} 个、合计丢弃 "
             f"{cp5.get('total_dropped_cash')} 起(披露) -> {'PASS' if cp5.get('ok') else 'FAIL'}")
    cd = checks.get("check_determinism_full", {})
    L.append(f"4. 确定性:全量 {cd.get('n_cells')} 格双跑逐位对拍,不一致 "
             f"{cd.get('n_mismatch')} 格 -> {'PASS' if cd.get('ok') else 'FAIL'}"
             "(落 detcmp.log)")
    L.append("5. 继承断言:资金守恒逐日流水回放零容差、入场日>事件日因果断言、"
    ...
    L.append("")
    L.append("## 2. 终审结果:三条线过线格数与名单(README §一/§二)")
    L.append("")
    L.append(f"- 主线 = 口径 D 纯现金字面版(excess_cash/资金利用率 > +10pp 且 "
             f"资金利用率 ≥ 0.5 且 sharpe_cash > 0.5):**过线 {verdict['n_passed_main']} / "
             f"{verdict['n_cells']} 格**")
    L.append(f"- 参考列 1 = 混合口径变体(excess_idx/资金利用率 > +10pp 且 资金利用率 ≥ 0.5 "
             f"且 sharpe_cash > 0.5):过线 {verdict['n_passed_mixed']} / {verdict['n_cells']} 格")
    ...
    for tag, key in (("主线", "passed_main_cells"),
        ...
        L.append("")
    # ---- 头部格(主线排序)与离线缺口量化 ----
    L.append("## 3. 头部格(按主线单位在投超额排序,前 10)与离线缺口量化")
    L.append("")
    ...
    if verdict["n_passed_main"] == 0:
        ...
        L.append("")
    # ---- P5 vs P1/P2 同(池,H,出场)对照(池×H 聚合;明细在 summary_elastic.csv)----
    L.append("## 4. P5 vs P1/P2 同 (池, H, 出场) 对照(P1/P2 数字取自 #32 冻结 summary)")
    L.append("")
    L.append("接入与资金利用率对照(池×H 聚合;P1/P2 为其五个挑选规则格的均值):")
    L.append("")
    L.append("| 池 | H | P5 接入合计(35 格) | P1 接入合计(五挑选均值) | "
             "P2 接入合计(五挑选均值) | P5 覆盖率 | P1 覆盖率 | P2 覆盖率 | "
             "P5 资金利用率均值 | P1 资金利用率均值 | P2 资金利用率均值 |")
    L.append("|---|---|---|---|---|---|---|---|---|---|")
    cmp_rows = []
    for (pool, H), g5 in summary.groupby(["pool", "H"]):
        gb = base[(base["pool"] == pool) & (base["H"] == H)]
        g1 = gb[gb["pos"] == "P1"]
        g2 = gb[gb["pos"] == "P2"]
        e5, e1, e2 = g5["entered"].sum(), g1["entered"].mean(), g2["entered"].mean()
        L.append(f"| {pool} | {H} | {int(e5)} | {e1:.1f} | {e2:.1f} | "
                 f"{g5['coverage'].mean():.4f} | {g1['coverage'].mean():.4f} | "
                 f"{g2['coverage'].mean():.4f} | {g5['capital_utilization'].mean():.4f} | "
                 f"{g1['capital_utilization'].mean():.4f} | {g2['capital_utilization'].mean():.4f} |")
    L.append("")
    L.append("超额对照(池×H 内 35 出场格的最优值;P1/P2 为五个挑选规则 × 35 出场的最优值):")
    L.append("")
    L.append("| 池 | H | P5 最优净年化超额(纯现金) | P1 最优 | P2 最优 | "
             "P5 最优净年化超额(判活) | P1 最优 | P2 最优 |")
    L.append("|---|---|---|---|---|---|---|---|")
    for (pool, H), g5 in summary.groupby(["pool", "H"]):
        gb = base[(base["pool"] == pool) & (base["H"] == H)]
        g1 = gb[gb["pos"] == "P1"]
        g2 = gb[gb["pos"] == "P2"]
        L.append(f"| {pool} | {H} | {g5['excess_cash'].max():+.4f} | "
                 f"{g1['excess_cash'].max():+.4f} | {g2['excess_cash'].max():+.4f} | "
                 f"{g5['excess_idx'].max():+.4f} | {g1['excess_idx'].max():+.4f} | "
                 f"{g2['excess_idx'].max():+.4f} |")
    L.append("")
    L.append("口径说明:P5 无挑选维(sel 段固定 S0),本节只表述信号池层面的接入与收益变化;"
             "池内排序增益(挑选规则价值)只适用于 #32 的 S1~S5 对照数字,两者分开表述不混比。")
    L.append("")
    # ---- 爆发日接入与摊薄行为统计 ----
    L.append("## 5. 爆发日接入与摊薄行为统计")
    L.append("")
    L.append("池级信号日分布(信号池层面事实,与出场无关):")
    L.append("")
    L.append("| 池 | 信号日数 | 信号数>10 的信号日占比 | 信号数>20 的信号日占比 | "
             "最大单日信号数 | 最大摊薄下单仓名义金额(元) |")
    L.append("|---|---|---|---|---|---|")
    for p in POOLS:
        pg = _G["pools"][p]
        sizes = np.array([len(v) for v in pg["by_date"].values()])
        nd = len(sizes)
        gt10 = float((sizes > 10).sum() / nd) if nd else 0.0
        gt20 = float((sizes > 20).sum() / nd) if nd else 0.0
        smax = int(sizes.max()) if nd else 0
        L.append(f"| {p} | {nd} | {gt10 * 100:.2f}%信号日信号数>10 | "
                 f"{gt20 * 100:.2f}%信号日信号数>20 | {smax} | "
                 f"{INIT / max(10, smax):,.0f} |")
    L.append("")
    n_cash = int((summary["dropped_cash"] > 0).sum())
    tot_cash = int(summary["dropped_cash"].sum())
    dc = summary["p5_days_cashbound"] / summary["signal_days"]
    dd = summary["p5_days_diluted"] / summary["signal_days"]
    L.append(f"- 全部接入:dropped_slot_full 恒 0(README §五.2 硬断言),当日池内信号无仓位数上限全接入。")
    L.append(f"- 现金丢弃:dropped_cash>0 的格 {n_cash} / {len(summary)} 个,"
             f"合计丢弃 {tot_cash} 起入场(现金再摊薄后不足整手)。")
    L.append(f"- 现金再摊薄触发(可用现金 ÷ S 紧于名义金额)的信号日占比:"
             f"{float((dc > 0).mean()) * 100:.2f}% 的格出现过现金再摊薄;"
             f"格均 {float(dc.mean()) * 100:.2f}% 的信号日触发。")
    L.append(f"- 名义摊薄(信号数>10,单仓名义 <10 万)的信号日占比:格均 "
             f"{float(dd.mean()) * 100:.2f}% 的信号日触发(池级口径见上表)。")
    L.append("")
    L.append("分池现金丢弃明细(各池 105 格聚合):")
    L.append("")
    L.append("| 池 | dropped_cash 合计 | 占信号总人次比 | entered 合计 |")
    L.append("|---|---|---|---|")
    for p in POOLS:
        g = summary[summary["pool"] == p]
        tot_sig = int(g["total_signals"].sum())
        dc5 = int(g["dropped_cash"].sum())
        L.append(f"| {p} | {dc5} | {dc5 / tot_sig * 100:.2f}% | "
                 f"{int(g['entered'].sum())} |")
    L.append("")
    ...
    for line in inherited_docstring_decisions():
        L.append(line)
    L.append("")
    L.append("本实验 P5 增量新增:")
    L.append("")
    for line in elastic_docstring_decisions():
        L.append(line)
    L.append("")
...
def elastic_docstring_decisions() -> list[str]:
    """P5 增量解释性决断(与头部 docstring E-1~E-10 逐字对应)。"""
    return [
...
# ---------------------------------------------------------------- 主流程
def main() -> None:
    ap = argparse.ArgumentParser(description="背离v6 R1 弹性仓位(P5) + 口径D 终审"
                                             "(预登记 README.md)")
    ...
    hb.start()
    log(f"ELASTIC POS TRIAL START | mode={args.mode} | "
    ...
    if not checks["check0_caliber_recompute"]["ok"]:
        log("CHECK0 FAIL —— 口径复算不逐位复现,停止(README §五.1:两数必须逐位复现)")
        _HB["stop"] = True
        sys.exit(1)
    ...
    est_full_h = mean_t * 630 / n_workers / 3600
    decision = f"全量 630 格(预估墙钟 {est_full_h:.2f} 小时,{n_workers} 进程并行)"
    ...
    with open(OUT_DIR / "benchmark.txt", "w", encoding="utf-8") as f:
        f.write("背离v6 R1 弹性仓位(P5) 单格耗时基准\n")
        f.write(f"日期: {time.strftime('%Y-%m-%d %H:%M:%S')}\n")
        ...
        f.write(f"\n单格耗时: 均值 {mean_t:.3f} 秒 / 中位 {med_t:.3f} 秒 / "
                f"最大 {max_t:.3f} 秒\n")
        f.write(f"预估全量 630 格墙钟: {est_full_h:.2f} 小时\n")
    ...
    checks["check3_causality"] = dict(
        ok=True, note="入场日>事件日硬断言;E2/E7 ATR ≤t−1;"
                      "P5 信号日决策只用信号日及之前信息(结构性;cash 为上一交易行收盘值)")
    ...
    if args.mode == "benchmark":
        _HB["stop"] = True
        log(f"ELASTIC POS TRIAL BENCHMARK DONE ({time.time() - _HB['t0']:.0f}s)")
        return

    # ---------------- 全量 630 格双跑(README §五.4 逐位确定性)----------------
    cells = enumerate_grid()
    assert len(cells) == 630, f"网格 {len(cells)} != 630"
    log(f"[full] 网格 {len(cells)} 格 = 池6 × H3 × 出场35 × P5")
    rows_full = run_cells_parallel(cells, write=True,
                                   stage=f"full pass1: {len(cells)} 格")
    # 自检 2/3:P5 无拒仓 + 对 #32 P1 单调性(硬断言,不过即中断)
    checks["checkP5"] = selfcheck_p5(rows_full)
    ...
    _HB["stop"] = True
    log(f"ELASTIC POS TRIAL DONE ({time.time() - _HB['t0']:.0f}s)")
