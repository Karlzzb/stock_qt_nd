#!/usr/bin/env python3
"""背离信号两变体 × 三出场 预登记回测矩阵执行器 N=10 增补轮（README.md「预登记增补 2026-09-04」A 节）。

矩阵：{events_v1, events_v2} × {A13, B15, E1-H12}，n_slots=10（本金 100 万不变，单仓约 10 万），
窗口 2026-01-01..2026-08-31，基准 000905.SH（index_metrics 口径，同 T8/T9 与上轮 N=3）。
除 n_slots 与产物路径（runs_n10/、summary_n10.csv、run_matrix_n10.log）外，
逐行复用上轮 run_matrix.py 口径。硬断言：prob 无缺失且全正、(ts_code,event_date) 无重复、
...
LOG_PATH = os.path.join(BT_DIR, "run_matrix_n10.log")
RUNS_DIR = os.path.join(BT_DIR, "runs_n10")
...
def main() -> None:
    ...
    log(f"MATRIX RUN START window=[{START}..{END}] bench={BENCH} "
        f"n_slots={N_SLOTS} configs={list(CONFIGS)} signals={list(EVENTS)}")
    ...
    for sig_name, ev_path in EVENTS.items():
        ...
        for cfg_name, spec in CONFIGS.items():
            ...
            out_dir = os.path.join(RUNS_DIR, f"{sig_name}__{cfg_name}")
    ...
    summary_path = os.path.join(BT_DIR, "summary_n10.csv")
