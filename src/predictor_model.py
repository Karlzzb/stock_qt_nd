...
class PriceChangePredictor:
    ...
    def load_models(self):
        ...
        try:
            ...
            # 加载特征列表
            with open(f"{self.model_dir}/selected_features.json", "r", encoding='utf-8') as f:
    ...
    def predict_proba(self, raw_features):
        ...
        # 缺失值填充
        X_feature_scaled = pd.DataFrame(
            self.imputer.transform(X),
            columns=X.columns
        )
        # 特征标准化
        # X_feature_scaled = self.feature_scaler.transform(X_imp)
        ...
        test_preds_lgb = np.average(
            [model.predict_proba(X_feature_scaled)[:, 1] for model in self.lgb_models],
            axis=0,
            weights=weights
        )

        # === 构造 stacking 特征 ===
        X_stack_test = X_feature_scaled.copy()
        ...
        X_test_scaled = self.scaler.transform(X_stack_test)
        if model_config.USE_PCA:
            X_test_scaled = self.dim_reducer.transform(X_stack_test)
    ...
    def _prepare_features(self, data):
        ...
        features = data[self.selected_features].astype(np.float32)
