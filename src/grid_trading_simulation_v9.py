...
def simple_run(initial_capital, strategy_name, strategy_params, full_data):
    ...
    # V9 新增参数
    strategy.recent_rise_n = strategy_params['recent_rise_n']
    strategy.recent_rise_pct = strategy_params['recent_rise_pct']
    strategy.rq_window = strategy_params['rq_window']
    strategy.rq_shrink_threshold = strategy_params['rq_shrink_threshold']
    strategy.rq_recover_threshold = strategy_params['rq_recover_threshold']
    ...
    if not trade_log.empty:
        ...
        asset_curve.to_csv(OUTPUT_DIR / f'simple_run_grid_v9_asset_log_{strategy_name}.csv', index=False)
        trade_log.to_csv(OUTPUT_DIR / f'simple_run_grid_v9_trade_log_{strategy_name}.csv', index=False)
...
def run_concurrent(init_capital, data):
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
        # 准备任务
        futures = []
        for name, params in model_config.STRATEGY_PARAMS_CANDIDATES_V9.items():
        ...
        for future in concurrent.futures.as_completed(futures):
            try:
                ...
                hold_analyzer(version="v9", param_suffix=strategy_name)
                profit_analyzer(version="v9", param_suffix=strategy_name)
                return_analyzer(version="v9", param_suffix=strategy_name)
                trades_analyzer(version="v9", param_suffix=strategy_name)
                # correlation_analyzer(version="v9", param_suffix=strategy_name)
        ...
        if len(all_analysis_result) > 0:
            analysis_txt = RESULT_DIR / f'strategy_compare_v9_full_analysis.txt'
        ...
        if len(analysis_df_list) > 0:
            ...
            analysis_df = RESULT_DIR / f'strategy_compare_v9_full_analysis.csv'
            os.makedirs(RESULT_DIR, exist_ok=True)
