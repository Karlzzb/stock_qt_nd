...
class TopKRankStrategy:
    def __init__(self, initial_capital=1000000, max_positions=10):
        ...
        # 5. 核心参数
        self.TOP_K_BUY = 0.05
        self.TOP_K_HOLD = 0.15
        self.min_probability = 0.45 #每天过滤掉一批比较低的预测候选股票
    ...
    def run(self, df):
        ...
        for i, today in enumerate(tqdm(dates)):
            ...
            # 1. 对预测候选集合进行过滤与排序
            ranked_df = self._rank_candidates(daily_data)

            # 2. 卖出/管理持仓
            self._manage_positions(ranked_df, daily_data, today)

            # 3. 买入新仓（下一个交易日的开仓，所以先卖出/管理持仓）
            self._open_new_positions(ranked_df, today, next_day)
    ...
    def _rank_candidates(self, daily_data):
        """
         当天的预测候选集合的过滤与排序
        """
        candidates = daily_data[~daily_data.index.isin(self.positions.keys())
                                & daily_data['y_pred_proba'].notna()].copy()
        if ~candidates.empty:
            # 按 y_pred_proba 降序排序
            if candidates.index.name == 'code' or 'code' not in candidates.columns:
                candidates = candidates.reset_index()  # 将索引转为列，索引名称变为'code'列
            candidates = candidates[candidates['y_pred_proba'] >= self.min_probability]  # 绝对阈值
            candidates = candidates.sort_values("y_pred_proba", ascending=False).reset_index(drop=True)
            candidates["rank"] = candidates.index + 1
            candidates["rank_pct"] = candidates["rank"] / len(candidates)
        return candidates

    def _candidates_plus_positions_rank(self, ranked_df):
        """
         当天的预测候选集合+持仓集合 的 排名
        """
        # 1. 从ranked_df提取候选股
        # 假设ranked_df的索引是股票代码，有y_pred_proba列
        candidates_series = pd.Series(
            dict(zip(ranked_df['code'], ranked_df['y_pred_proba']))
        )

        # 2. 从持仓中提取数据
        positions_series = pd.Series({
            code: pos['y_pred_proba']
            for code, pos in self.positions.items()
        })

        # 3. 合并两个Series
        combined_series = pd.concat([candidates_series, positions_series])
        ...
        # 4. 计算排名
        # 转换为DataFrame进行排序
        df = combined_series.reset_index()
        df.columns = ['code', 'y_pred_proba']
        ...
        # 5. 创建映射
        rank_map = dict(zip(df['code'], df['rank_pct']))

        return rank_map

    def _manage_positions(self, ranked_df, daily_data, today):

        rank_map = self._candidates_plus_positions_rank(ranked_df)
        codes_to_remove = []
        ...
        for code, pos in self.positions.items():
            ...
            # --- 4. 提出排名过期的股票 ---
            if len(rank_map) > 0:
                rank_pct = rank_map.get(code, 1.0)
                if rank_pct > self.TOP_K_HOLD:
                    self._execute_trade(today, code, 'RANK_EXIT', curr_c, -pos['shares'], pos)
                    codes_to_remove.append(code)
                    continue
    ...
    def _open_new_positions(self, ranked_df, today, next_day):
        ...
        # 1. 从候选集合中过滤出TOP_K
        buy_pool = ranked_df[
            (ranked_df["rank_pct"] <= self.TOP_K_BUY) &
            (~ranked_df["code"].isin(self.positions)) &
            # FIXED BUG  这里过滤，而不在数据预处理过滤，避免simulation与 real_world 执行不一致的问题。
            (ranked_df['close'] <= model_config.AFFORDABLE_PRICE)
            ]
        ...
        for _, row in buy_pool.iterrows():
            ...
            logger.debug(f"挂单: {row['code']} 信号价{signal_price:.2f} 计划{planned_shares}股 冻结{required_cash:.2f}")
        ...
        for order in orders:
            ...
            # 条件1: 次日最低价必须 <= signal_price 才可能成交
            if next_low > signal_price:
                logger.info(f"{code} 未成交: next_low {next_low:.2f} > signal_price {signal_price:.2f}")
                # TODO 这里直接按照开盘价格买入
                signal_price = next_open
                # continue
...
def simple_run(initial_capital, strategy_name, strategy_params, full_data):
    ...
    strategy = TopKRankStrategy(initial_capital=initial_capital, max_positions=5)
    ...
    strategy.TOP_K_BUY = strategy_params['top_k_buy']
    strategy.TOP_K_HOLD = strategy_params['top_k_hold']
    strategy.min_probability = strategy_params['min_probability']
    ...
    if not trade_log.empty:
        asset_curve.to_csv(RESULT_DIR / f'simple_run_grid_v7_asset_log_{strategy_name}.csv', index=False)
        trade_log.to_csv(RESULT_DIR / f'simple_run_grid_v7_trade_log_{strategy_name}.csv', index=False)

        # 所有开仓动作
        open_actions = ['OPEN_BUY', 'GRID_ADD']
        # 所有平仓动作
        close_actions = ['TAKE_PROFIT', 'INTRADAY_STOP_LOSS', 'TIME_EXIT', 'RANK_EXIT']
        ...
        # analysis_dir = RESULT_DIR / f"portfolio_analysis_charts_v7_{strategy_name}"
        # analysis_txt = analysis_dir / f'simple_run_grid_v7_summary_{strategy_name}.txt'
...
def run_concurrent(init_capital, data):
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
        # 准备任务
        futures = []
        for name, params in model_config.STRATEGY_PARAMS_CANDIDATES_V7.items():
        ...
        for future in concurrent.futures.as_completed(futures):
            try:
                ...
                # hold_analyzer(version="v7", param_suffix=result)
                # profit_analyzer(version="v7", param_suffix=result)
                # return_analyzer(version="v7", param_suffix=result)
                # trades_analyzer(version="v7", param_suffix=result)
                # correlation_analyzer(version="v7", param_suffix=result)
        ...
        if len(all_analysis_result) > 0:
            analysis_txt = RESULT_DIR / f'simple_run_grid_v7_full_params_analysis.txt'
