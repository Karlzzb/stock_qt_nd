...
try:
    # 从原文件中导入策略类和数据加载函数
    from grid_trading_simulation_v8 import SmartSniperStrategy, data_process, logger
except ImportError as e:
    print(f"导入 1_grid_trading_simulation_v8.py 失败。请检查文件是否在同一目录。错误: {e}")
    sys.exit(1)
# ----------------------------------------------------
...
def concurrent_parameter_optimization(df_result, init_capital):
    ...
    num_workers = 12 #max(1, os.cpu_count() - 1)
...
def finding_best_params_concurrent(init_capital):
    ...
    if not optimization_results.empty:
        ...
        # 保存结果到文件
        output_file = 'parameter_optimization_results_concurrent_v8.csv'
