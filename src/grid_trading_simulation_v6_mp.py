...
try:
    # 从原文件中导入策略类和数据加载函数
    from grid_trading_simulation_v6 import SmartSniperStrategy, data_process, logger
except ImportError as e:
    print(f"导入 1_grid_trading_simulation_v6.py 失败。请检查文件是否在同一目录。错误: {e}")
    sys.exit(1)
# ----------------------------------------------------
...
def _run_single_backtest(params, param_names, df_result, initial_capital):
    ...
    try:
        ...
        strategy = SmartSniperStrategy(initial_capital=initial_capital, max_positions=param_dict['max_positions'])
        ...
        strategy.top_kop = param_dict['top_k']
...
def concurrent_parameter_optimization(df_result, initial_capital):
    ...
    num_workers = 26 #max(1, os.cpu_count() - 1)
...
def finding_best_params_concurrent(initial_capital):
    """
    整合数据加载、并发优化和结果分析的函数。
    """
    ...
    # 参数优化部分 (并发)
    # ==========================================
    optimization_results = concurrent_parameter_optimization(df_result, initial_capital)
    ...
    if not optimization_results.empty:
        ...
        # 保存结果到文件
        output_file = 'parameter_optimization_results_concurrent_top_k_v6.csv'
