...
# 现在导入配置，让它读取更新后的环境变量
from config.settings import MODEL_DIR, REAL_TRADING_DIR, DAILY_FEATURE_DIR, ST_FILTER_DATA_DIR, STOCK_ND_CSV_DIR, STOCK_DATA_DIR, DATASET_DIR, RESULT_DIR
            ...
            if self.positions:
                ...
                for code, pos in self.positions.items():
                    pos_data = {
                        ...
                        'should_sell_date': pos.get('entry_date') + pd.Timedelta(days=STRATEGY_PARAMS['max_hold_days']) if pos.get('entry_date') else None,
...
class SmartSniperInvestor:
    ...
    def __init__(self, initial_capital: float = 200000, max_positions: int = 5, base_dir = REAL_TRADING_DIR):
        ...
        # V12 波动率自适应参数
        self.use_volatility_adaptive = STRATEGY_PARAMS.get('use_volatility_adaptive', True)
        self.vol_lookback = STRATEGY_PARAMS.get('vol_lookback', 14)
        self.vol_high_thresh = STRATEGY_PARAMS.get('vol_high_thresh', 2.5)
        self.vol_low_thresh = STRATEGY_PARAMS.get('vol_low_thresh', 0.6)
        self.vol_profit_mult = STRATEGY_PARAMS.get('vol_profit_mult', 1.5)
        self.vol_stop_mult = STRATEGY_PARAMS.get('vol_stop_mult', 1.3)
        self.low_vol_profit_mult = STRATEGY_PARAMS.get('low_vol_profit_mult', 0.80)
        self.use_market_vol = STRATEGY_PARAMS.get('use_market_vol', False)
    ...
    def _generate_buy_suggestions(self, daily_data: pd.DataFrame, predict_date: datetime) -> List[Dict]:
        ...
        # stock_filter.filter() expects symbol in index, so set 'code' as index
        candidates_filter = candidates.set_index('code')
        candidates = self.stock_filter.filter(candidates_filter, trade_date)
        # Restore as regular DataFrame with 'code' column
    ...
    def _generate_daily_report(self, date: datetime, price_data: pd.DataFrame,
        ...
        with open(report_file, 'w', encoding='utf-8') as f:
            f.write(f"📊 智能狙击手投资日报（人工执行版本 V12）\n")
...
def run(predict_date = datetime.now(), feature_date = datetime.now(), real_trading_dir  = REAL_TRADING_DIR):
    ...
    logger.debug("=" * 60)
    logger.debug(f"🤖 智能狙击手投资系统启动（人工执行版本 V12）")
    logger.debug("=" * 60)
...
if __name__ == "__main__":
    ...
    # 创建参数解析器
    # parser = argparse.ArgumentParser(description='SmartSniper 实盘投资脚本 V12')
