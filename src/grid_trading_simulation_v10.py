...
def simple_run(initial_capital, strategy_name, strategy_params, full_data):
    ...
    # V10 新增参数
    strategy.trailing_offset = strategy_params['trailing_offset']
    strategy.trailing_pct = strategy_params['trailing_pct']
    strategy.use_partial_take_profit = strategy_params['use_partial_take_profit']
    strategy.partial_profit_1 = strategy_params['partial_profit_1']
    strategy.partial_profit_2 = strategy_params['partial_profit_2']
    strategy.partial_ratio_1 = strategy_params['partial_ratio_1']
    strategy.use_trailing_stop = strategy_params['use_trailing_stop']
    strategy.trailing_stop_pct = strategy_params['trailing_stop_pct']
    ...
    if not trade_log.empty:
        ...
        asset_curve.to_csv(OUTPUT_DIR / f'simple_run_grid_v10_asset_log_{strategy_name}.csv', index=False)
        trade_log.to_csv(OUTPUT_DIR / f'simple_run_grid_v10_trade_log_{strategy_name}.csv', index=False)
        ...
        # analysis_dir = RESULT_DIR / f"portfolio_analysis_charts_v10_{strategy_name}"
        # analysis_txt = analysis_dir / f'simple_run_grid_v10_summary_{strategy_name}.txt'
...
# V10 参数网格 - V8参数固定为最佳值(参数55)
V10_BEST_V8_PARAMS = model_config.STRATEGY_PARAMS_CANDIDATES_V8["参数55"]
...
def build_v10_params(trailing_offset, trailing_pct, use_partial_take_profit,
                      partial_profit_1, partial_profit_2, partial_ratio_1,
                      use_trailing_stop, trailing_stop_pct):
    """构建V10参数组合，合并V8最佳参数和V10新参数"""
    params = V10_BEST_V8_PARAMS.copy()
    params.update({
        'trailing_offset': trailing_offset,
        'trailing_pct': trailing_pct,
        'use_partial_take_profit': use_partial_take_profit,
        'partial_profit_1': partial_profit_1,
        'partial_profit_2': partial_profit_2,
        'partial_ratio_1': partial_ratio_1,
...
def run_concurrent(init_capital, data):
    ...
    # 生成所有V10参数组合
    all_combinations = []
    for trailing_offset in param_grid['trailing_offset']:
        for trailing_pct in param_grid['trailing_pct']:
            for use_partial_take_profit in param_grid['use_partial_take_profit']:
                for partial_profit_1 in param_grid['partial_profit_1']:
                    for partial_profit_2 in param_grid['partial_profit_2']:
                        for partial_ratio_1 in param_grid['partial_ratio_1']:
                            for use_trailing_stop in param_grid['use_trailing_stop']:
                                for trailing_stop_pct in param_grid['trailing_stop_pct']:
                                    name = f"v10_to{trailing_offset}_tp{trailing_pct}_ptp{int(use_partial_take_profit)}_pp1{partial_profit_1}_pp2{partial_profit_2}_pr1{partial_ratio_1}_uts{int(use_trailing_stop)}_tsp{trailing_stop_pct}"
                                    params = build_v10_params(
                                        trailing_offset, trailing_pct, use_partial_take_profit,
                                        partial_profit_1, partial_profit_2, partial_ratio_1,
                                        use_trailing_stop, trailing_stop_pct
                                    )
                                    all_combinations.append((name, params))

    print(f"总共 {len(all_combinations)} 个参数组合")

    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
        # 准备任务
        futures = []
        for name, params in all_combinations:
        ...
        for future in concurrent.futures.as_completed(futures):
            try:
                ...
                hold_analyzer(version="v10", param_suffix=strategy_name)
                profit_analyzer(version="v10", param_suffix=strategy_name)
                return_analyzer(version="v10", param_suffix=strategy_name)
                trades_analyzer(version="v10", param_suffix=strategy_name)
                # correlation_analyzer(version="v10", param_suffix=strategy_name)
        ...
        if len(all_analysis_result) > 0:
            analysis_txt = RESULT_DIR / f'strategy_compare_v10_full_analysis.txt'
            os.makedirs(RESULT_DIR, exist_ok=True)
            with open(analysis_txt, 'w', encoding='utf-8') as f:
                f.write('\n'.join(all_analysis_result))
        if len(analysis_df_list) > 0:
            final_df = pd.concat(analysis_df_list, ignore_index=True)
            analysis_df = RESULT_DIR / f'strategy_compare_v10_full_analysis.csv'
            os.makedirs(RESULT_DIR, exist_ok=True)
...
if __name__ == "__main__":
    ...
    # 使用V10默认参数运行单个测试
    default_params = build_v10_params(
        trailing_offset=0.08,
        trailing_pct=0.20,
        use_partial_take_profit=False,
        partial_profit_1=0.12,
        partial_profit_2=0.25,
        partial_ratio_1=0.5,
        use_trailing_stop=False,
        trailing_stop_pct=0.20
    )
    simple_run(initial_capital=248526, strategy_name="v10_default", strategy_params=default_params, full_data=processed_data)
