...
class SmartSniperStrategyV13:
    """
    V13 策略：极端非对称仓位 + 波动率移动止盈 (专为高精度/低召回模型设计)
    """
    def __init__(self, initial_capital=1000000, max_positions=5, st_preloaded=None):
        self.initial_capital = initial_capital
        self.cash = initial_capital
        self.max_positions = max_positions # 默认极度聚焦 5 只

        # --- 基础参数 ---
        self.base_ratio = 1.0            
        self.target_profit = 0.50        # 放开基础止盈，让利润奔跑
        self.hard_stop_loss = -0.06      # 稍微收紧硬止损
        self.max_hold_days = 20          
        self.min_probability = 0.35      # 下放最低门槛，增加交易频率（通过分级仓位控制风险）
        ...
        # --- V13 核心：移动跟踪止损 (Trailing Stop) 默认开启 ---
        self.use_trailing_stop = True
        self.trailing_stop_pct = 0.10    # 从最高点回撤 10% 止盈出局
        self.trailing_activation = 0.08  # 利润达到 8% 后激活跟踪止损
    ...
    def _compute_atr(self, prices_df, symbol, current_date, window=14):
        ...
        if symbol not in prices_df: return None
    ...
    def _precompute_market_atr(self, today, prices_df):
        ...
        if self.atr_cache:
            ...
            for code in prices_df:
                ...
                if atr is not None and atr > 0: market_atrs.append(atr)
    ...
    def _get_volatility_multiplier(self, symbol, current_date, prices_df):
        if not self.use_volatility_adaptive: return 1.0
        individual_atr = self._compute_atr(prices_df, symbol, current_date, self.vol_lookback)
        if individual_atr is None: return 1.0
        market_avg_atr = self.market_avg_atr
        if market_avg_atr is None or market_avg_atr == 0: return 1.0
    ...
    def run(self, df, prices_df=None, show_progress=False, _timing_hook=None):
        ...
        for i, today in enumerate(iterator):
            ...
            for code, pos in self.positions.items():
                pos['volatility_mult'] = self._get_volatility_multiplier(code, today, prices_df)
    ...
    def _manage_positions(self, daily_data, today):
        ...
        for code, pos in self.positions.items():
            ...
            curr_h, curr_l, curr_o, curr_c = row['high'], row['low'], row['open'], row['close']
            days_held = (today - pos['entry_date']).days

            if days_held < 1: continue

            target_price, stop_price = self._get_adaptive_targets(pos)

            # 1. 硬止损
            if curr_o <= stop_price or curr_l <= stop_price:
                sell_price = min(curr_o if curr_o <= stop_price else stop_price, curr_h)
            ...
            if self.use_trailing_stop:
                ...
                # 只有利润达到激活线，才启动移动止损
                if highest_price >= pos['avg_cost'] * (1 + self.trailing_activation):
                    trailing_stop_price = highest_price * (1 - self.trailing_stop_pct)
                    if curr_c <= trailing_stop_price:
                        self._execute_trade(today, code, 'TRAILING_TAKE_PROFIT', curr_c, -pos['shares'], pos, days_held)
                        codes_to_remove.append(code)
                        continue

            # 3. 基础止盈 (兜底)
            if curr_h >= target_price:
                sell_price = min(max(curr_o, target_price), curr_h)
    ...
    def _open_new_positions(self, daily_data, today, next_day, prices_df=None):
        ...
        # 按绝对分数排序
        top_candidates = candidates.sort_values(['y_pred_proba', 'code'], ascending=[False, True]).head(available_slots * 2)
        ...
        for code, row in top_candidates.iterrows():
            ...
            # ==========================================
            if proba >= 0.80:
                weight_factor = 2.5  # S级绝杀：倾泻极其重仓
            elif proba >= 0.65:
                weight_factor = 1.5  # A级：标准偏重
            elif proba >= 0.50:
                weight_factor = 0.8  # B级：轻度参与
            else:
                weight_factor = 0.4  # C级：轻仓试错 (防闲置空转)

            base_budget_per_slot = available_cash / max(1, remaining_slots)
            budget = base_budget_per_slot * weight_factor * self.base_ratio
            ...
            vol_mult = self._get_volatility_multiplier(code, today, prices_df) if self.use_volatility_adaptive and prices_df is not None else 1.0

            orders.append({
                'code': code, 'entry_date': row.get('entry_date'), 'signal_price': signal_price,
                'planned_shares': planned_shares, 'next_open': row.get('next_open'),
                'next_low': row.get('next_low'), 'proba': proba, 'volatility_mult': vol_mult,
            })
        ...
        actual_cost = 0.0
        for order in orders:
            code, signal_price, planned_shares = order['code'], order['signal_price'], order['planned_shares']
            next_open, next_low = order.get('next_open'), order.get('next_low')

            if next_open is None or next_low is None or next_low > signal_price: continue
            if next_day is None or (order.get('entry_date') - next_day).days > 0: continue

            actual_price = signal_price
            order_cost = actual_price * planned_shares

            self.positions[code] = {
                'avg_cost': actual_price, 'shares': planned_shares, 'last_buy_price': actual_price,
                'entry_date': order.get('entry_date'), 'proba': order['proba'],
                'highest_price': actual_price, 'trailing_stop': 0, 'volatility_mult': order['volatility_mult'],  
            }
    ...
    def _log(self, date, code, action, price, shares, profit, profit_pct, days_held, proba):
        self.history.append({
            'date': date, 'code': code, 'action': action, 'price': price, 'shares': shares,
            'profit': profit, 'profit_pct': profit_pct, 'days_held': days_held, 'proba': proba
        })

    def _record_daily_asset(self, today, daily_data):
        mkt_val = sum(daily_data.loc[c, 'close'] * p['shares'] if c in daily_data.index else p['avg_cost'] * p['shares'] for c, p in self.positions.items())
