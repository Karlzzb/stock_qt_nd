...
import time
import logging
from config.settings import MODEL_DIR, DATASET_DIR, RESULT_DIR
...
try:
    # 从原文件中导入策略类和数据加载函数
    from grid_trading_simulation_v7 import TopKRankStrategy, data_process, logger
except ImportError as e:
    print(f"导入 1_grid_trading_simulation_v5.py 失败。请检查文件是否在同一目录。错误: {e}")
    sys.exit(1)
# ----------------------------------------------------
...
def _run_single_backtest(params, param_names, df_result, initial_capital):
    """
    ...
    try:
        # 1. 初始化策略
        # 必须传入 max_positions 因为 TopKRankStrategy.__init__ 中会用到
        strategy = TopKRankStrategy(initial_capital=initial_capital, max_positions=param_dict['max_positions'])
        ...
        strategy.TOP_K_HOLD =  param_dict['top_k_hold']
        strategy.TOP_K_BUY = param_dict['top_k_buy']
        ...
        closed_trades = trade_log[trade_log['action'].isin(['TAKE_PROFIT', 'INTRADAY_STOP_LOSS', 'TIME_EXIT', 'RANK_EXIT'])]
        ...
        logger.error(f"参数组合 {param_dict} 测试失败: {e}")
        return None


def concurrent_parameter_optimization(df_result, initial_capital):
    """
    ...
    param_grid = {
        'base_ratio': [1.0],
        'target_profit': [2.0, 10],
        'hard_stop_loss': [-0.02, -0.03, -0.04],
        'max_hold_days': [15, 18, 20, 25, 30],
        'max_positions': [2, 3, 4],
        'top_k_buy': [0.05, 0.06, 0.07, 0.08],
        'top_k_hold': [0.15, 0.16, 0.18, 0.2],
        'min_probability': [0.4, 0.5, 0.6, 0.7]
    }
    ...
    with ProcessPoolExecutor(max_workers=num_workers) as executor:
        ...
        futures = [
            executor.submit(_run_single_backtest, params, param_names, df_result, initial_capital)
...
def finding_best_params_concurrent(capital, data):
    ...
    # 参数优化部分 (并发)
    # ==========================================
    optimization_results = concurrent_parameter_optimization(data, capital)
